...
class TestAddWithIgnore:
    ...
    def test_add_refuses_ignored_file(self, tmp_path):
        ...
        assert "Use --force to add" in result.stdout
        
        # Verify it was NOT added
        tracked = load_tracked(ctx)
        assert "test.pyc" not in tracked.files
    
    def test_add_with_force_adds_ignored_file(self, tmp_path):
    ...
    def test_add_custom_ignore_patterns(self, tmp_path):
...
""")
    ...
    def test_add_multiple_with_some_ignored(self, tmp_path):
        ...
        assert result.exit_code == 0
        assert "Tracking 2 files" in result.stdout
        assert "+ main.py" in result.stdout
    ...
    def test_add_directory_ignores_patterns(self, tmp_path):
    ...
    def test_tracked_stays_tracked_even_if_later_ignored(self, tmp_path):
